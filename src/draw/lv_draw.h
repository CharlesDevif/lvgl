...
typedef enum {
    ...
    /** Cannot be completed, e.g. the layer buffer could not be allocated.
     * Removed like a finished task, so the frame completes without it
     * rather than not completing at all. */
