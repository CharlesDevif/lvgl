 ...
 */
/* Overridable: past the stencil, only 10240 bytes are left for every
 * nema_vg_path_create() and nema_vg_paint_create(), and FreeType outline mode
 * makes one of each per cached glyph. An exhausted pool is not reported --
 * nema_vg_path_create() does not check its allocator, and faults. */
